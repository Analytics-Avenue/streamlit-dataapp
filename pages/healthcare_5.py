...
# -------------------------
# Application Page (Single Scroll)
# -------------------------
st.header("Application — Full Setup")

# -------- Dataset Setup --------
st.subheader("1️⃣ Dataset Setup")
mode = st.radio("Select Dataset Mode", ["Default (URL)","Upload CSV","Upload + Mapping"],horizontal=True)
...
if len(num_cols) >= 2:
    x = st.selectbox("X axis for Scatter", num_cols, key="scatter_x")
    y = st.selectbox("Y axis for Scatter", num_cols, key="scatter_y")
    fig = px.scatter(df, x=x, y=y, color=cat_cols[0] if cat_cols else None)
    st.plotly_chart(fig, use_container_width=True)

# -------- ML Revenue Prediction --------
st.subheader("3️⃣ ML Revenue Prediction (Regression)")
if "Revenue" not in df.columns: st.info("Ensure 'Revenue' numeric column exists."); st.stop()
...
# -------- Automated Insights --------
st.subheader("4️⃣ Automated Insights")
insights=[]
