...
dataset_option = st.radio("Select Dataset Source", 
                          ["Default Dataset", "Upload CSV", "Manual Entry"])

if dataset_option == "Default Dataset":
    # Load your repo dataset here
    @st.cache_data
    def load_default():
        # Replace with your default dataset path
        return pd.DataFrame({
            "Campaign": ["A", "B", "C", "D"],
    ...
    cols = st.text_input("Enter column names separated by commas")
    if cols:
        cols_list = [c.strip() for c in cols.split(",")]
        rows = st.text_area("Enter rows (comma separated values per row, line per row)")
        if rows:
            data = []
...
if st.session_state.df is not None:
    ...
    if target:
        ...
        st.subheader("Pipeline & Model Setup")
