...
upload_option = st.sidebar.radio("Choose how to provide data:", ["Direct Upload (default columns)", "Upload and Map Columns"])
        ...
        for key, default_name in DEFAULT_COLUMNS.items():
            if default_name not in df.columns:
                user_columns[key] = st.sidebar.selectbox(
                    f"Select column for '{default_name}' (or leave blank)", options=[None]+list(df.columns), index=0
                )
            else:
...
# ------------------------
# Helper function for styling axes
# ------------------------
def style_axes(fig):
    fig.update_layout(
...
# ------------------------
    ...
    if mapped_df['spent'] is not None:
        st.metric("Total Spent (INR)", f"₹{mapped_df['spent'].sum():,.0f}")
    if mapped_df['impressions'] is not None:
        st.metric("Total Impressions", f"{mapped_df['impressions'].sum():,.0f}")
    if mapped_df['reach'] is not None:
        st.metric("Total Reach", f"{mapped_df['reach'].sum():,.0f}")
    ...
    if mapped_df['start'] is not None and mapped_df['spent'] is not None:
        ...
        fig = px.line(
            spend_over_time, x='start', y='spent', markers=True,
            text=spend_over_time['spent'].apply(lambda x: f"₹{x:,.0f}"), title="Campaign Spend Over Time",
            color_discrete_sequence=['#2E86C1']
        )
        fig.update_traces(textposition='top right')
        fig = style_axes(fig)
        st.plotly_chart(fig, use_container_width=True)
        st.markdown("**Purpose:** Shows spending trends over time.")
        st.info("**Quick Tip:** Look for peaks or drops to understand budget usage.")
    ...
    if mapped_df['campaign'] is not None and mapped_df['spent'] is not None:
        ...
        fig = px.bar(
            top_campaigns, x='spent', y='campaign', orientation='h',
        ...
        fig = style_axes(fig)
        st.plotly_chart(fig, use_container_width=True)
        st.markdown("**Purpose:** Highlights which campaigns receive the most budget.")
        st.info("**Quick Tip:** Use this to focus on key campaigns for analysis.")
...
# ------------------------
    ...
    if mapped_df['age'] is not None and mapped_df['gender'] is not None and mapped_df['clicks'] is not None:
        ...
        fig = style_axes(fig)
        st.plotly_chart(fig, use_container_width=True)
        st.markdown("**Purpose:** Identifies most engaged audience segments.")
        st.info("**Quick Tip:** Use for targeted messaging.")
    ...
    if mapped_df['city'] is not None and mapped_df['spent'] is not None:
        ...
        fig = px.bar(city_perf, x='city', y='spent', text=city_perf['spent'].apply(lambda x: f"₹{x:,.0f}"), color='city', title="Top Cities by Ad Spend", color_discrete_sequence=px.colors.sequential.Viridis)
        fig.update_traces(textposition='outside')
        fig = style_axes(fig)
        st.plotly_chart(fig, use_container_width=True)
        st.markdown("**Purpose:** Shows cities with highest ad spend.")
        st.info("**Quick Tip:** Focus analysis on high spend cities.")
...
# ------------------------
    ...
    if mapped_df['ad_name'] is not None and mapped_df['clicks'] is not None:
        ...
        fig = style_axes(fig)
        st.plotly_chart(fig, use_container_width=True)
        st.markdown("**Purpose:** Highlights ads generating highest clicks.")
        st.info("**Quick Tip:** Focus on high CTR & high clicks ads.")

        # CPC vs CTR
        if 'cpc' in ad_perf.columns and 'ctr' in ad_perf.columns:
            fig = px.scatter(ad_perf, x='cpc', y='ctr', color='spent', text=ad_perf['spent'].apply(lambda x: f"₹{x:,.0f}"), hover_name='ad_name', title="CPC vs CTR")
...
# ------------------------
    ...
    if all(mapped_df[col] is not None for col in video_cols) and mapped_df['ad_name'] is not None:
        ...
        fig = style_axes(fig)
        st.plotly_chart(fig, use_container_width=True)
        st.markdown("**Purpose:** Shows how viewers progress through video stages.")
        st.info("**Quick Tip:** Identify drop-off points to optimize content.")
    ...
    if mapped_df['thruplays'] is not None and mapped_df['cost_per_thruplay'] is not None and mapped_df['impressions'] is not None:
        ...
        st.info("**Quick Tip:** Focus on low Cost per ThruPlay with high ThruPlays and large impressions.")
